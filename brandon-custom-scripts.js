...
// Version: 3.0.0 (Corrected Structure)
// Date: 2025-06-22
...
function logDebug(...args) {
  ...
  function initializeBrandonAnimations() {
    ...
    animatedModules.forEach(module => {
      ...
      const originalSpan = link.querySelector('span');
      let originalColor = 'inherit';

      if (originalSpan && originalSpan.style.color) {
          originalColor = originalSpan.style.color;
      } else {
          originalColor = window.getComputedStyle(link).color;
      }
  ...
  /**
   * Initializes the scroll-based animation for the hero text.
   * This is now correctly placed inside the main script wrapper.
   */
  function initializeHeroScrollAnimation() {
    ...
    // A small delay to give fonts a moment to load, addressing the console warning.
    gsap.delayedCall(0.1, () => {
      ...
      // Make sure the container is visible before starting
  ...
  function initializeBrandonComponents() {
    brandonLog("Initializing Brandon Components (v3.0.0)");
    ...
    initializeHeroScrollAnimation(); // <-- Correctly called here
